...
class User {
   ...
   * @throws {NotFoundError} - If no user is found with the specified username.
   */
  ...
  static async applyToJob(username, id) {
    ...
    const jobId = appRes.rows;

    if (!jobId) throw new NotFoundError(`No user: ${username}`);
   ...
   * @returns {string[]} - An array of job IDs the user has applied for.
   * @throws {NotFoundError} - If no user is found with the specified username.
   */
  ...
  static async getApplications(username) {
    ...
    const applications = userApplications.rows.map((row) => row.job_id);
